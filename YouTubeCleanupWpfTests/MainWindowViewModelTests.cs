...
public class MainWindowViewModelTests(ITestOutputHelper testOutputHelper)
{
    ...
    {
            testOutputHelper.WriteLine($"mainWindowViewModel.Videos: {JsonConvert.SerializeObject(mainWindowViewModel.Videos.Select(x => new {x.Id, x.Title}), Formatting.Indented)}");
            testOutputHelper.WriteLine($"expected: {JsonConvert.SerializeObject(expectedVideos.Select(x => new {x.Id, x.Title}), Formatting.Indented)}");
        }
