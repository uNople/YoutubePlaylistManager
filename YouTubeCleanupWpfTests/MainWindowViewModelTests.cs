...
public class MainWindowViewModelTests(ITestOutputHelper testOutputHelper)
{
#if DEBUG
    [Theory, AutoNSubstituteData]
    public async Task When_nothing_exists_Then_playlists_and_filters_and_videos_are_empty(
    ...
    [Theory, AutoNSubstituteData]
    public async Task
        When_new_playlist_created_on_YouTube_and_refresh_happens_Then_playlist_inserted_into_ui_in_the_correct_location(
        ...
        [NoAutoProperties] MainWindowViewModel mainWindowViewModel)
    {
        ...
        var playlistItems = new List<PlaylistItemData>()
            { new() { VideoId = videoData.Id, PlaylistDataId = playlistData.Id } };
    ...
    [Theory, AutoNSubstituteData]
    public async Task
        When_video_added_to_playlist_on_YouTube_and_refresh_happens_Then_video_inserted_into_ui_in_the_correct_location(
            [Frozen] IYouTubeCleanupToolDbContext youTubeCleanupToolDbContext,
            [Frozen] IYouTubeCleanupToolDbContextFactory youTubeCleanupToolDbContextFactory,
            WpfPlaylistData playlistData,
            [NoAutoProperties] MainWindowViewModel mainWindowViewModel)
    {
        ...
        {
            new() { VideoId = videoData.Id, PlaylistDataId = playlistData.Id },
            new() { VideoId = videoDataTwo.Id, PlaylistDataId = playlistData.Id },
            new() { VideoId = videoDataThree.Id, PlaylistDataId = playlistData.Id }
        };
        playlistData.PlaylistItems = playlistItems;
        youTubeCleanupToolDbContext.GetPlaylists().Returns(new List<PlaylistData> { playlistData });
        var playlistDataForVideo4 = new PlaylistItemData
            { VideoId = videoDataFour.Id, PlaylistDataId = playlistData.Id };
        youTubeCleanupToolDbContext.GetPlaylistItems().Returns(playlistItems,
            new List<PlaylistItemData>(playlistItems) { playlistDataForVideo4 });
    ...
    private void LogVideos(MainWindowViewModel mainWindowViewModel, List<WpfVideoData> expectedVideos)
    {
        testOutputHelper.WriteLine(
            $"mainWindowViewModel.Videos: {JsonConvert.SerializeObject(mainWindowViewModel.Videos.Select(x => new { x.Id, x.Title }), Formatting.Indented)}");
        testOutputHelper.WriteLine(
            $"expected: {JsonConvert.SerializeObject(expectedVideos.Select(x => new { x.Id, x.Title }), Formatting.Indented)}");
    }
