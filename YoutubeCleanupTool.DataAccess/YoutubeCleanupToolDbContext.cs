...
namespace YouTubeCleanupTool.DataAccess
{
    public class YouTubeCleanupToolDbContext : DbContext, IYouTubeCleanupToolDbContext
    {
        ...
        public async Task<List<PlaylistItemData>> GetPlaylistItems() => await PlaylistItems.OrderBy(x => x.Id).ThenBy(x => x.Position).ToListAsync();
        public async Task<List<PlaylistItemData>> GetPlaylistItems(string playlistId) => await PlaylistItems.Where(x => x.PlaylistDataId == playlistId).OrderBy(x => x.Id).ThenBy(x => x.Position).ToListAsync();
