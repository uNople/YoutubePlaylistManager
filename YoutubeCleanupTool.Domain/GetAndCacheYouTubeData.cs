    ...
    : IGetAndCacheYouTubeData
{
    ...
    {
            var context = youTubeCleanupToolDbContextFactory.Create();
            await foreach (var playlist in youTubeApi.GetPlaylists().WithCancellation(cancellationToken))
            {
    ...
    private async Task GetPlaylistItems(IYouTubeCleanupToolDbContext context, Func<IData, InsertStatus, CancellationToken, Task> callback, PlaylistData playlist, CancellationToken cancellationToken)
    {
            ...
            await foreach (var playlistItem in youTubeApi.GetPlaylistItems(playlist.Id, RemovePlaylist).WithCancellation(cancellationToken))
            {
    ...
    public async Task GetVideos(Func<IData, InsertStatus, CancellationToken, Task> callback, bool getAllVideos, CancellationToken cancellationToken)
    {
            ...
            await foreach (var video in youTubeApi.GetVideos(videosToGet).WithCancellation(cancellationToken))
            {
                cancellationToken.ThrowIfCancellationRequested();
    ...
    private async Task<byte[]> GetThumbnail(CancellationToken cancellationToken, VideoData video)
    {
            try
            {
                return await httpClientWrapper.GetByteArrayAsync(video.ThumbnailUrl, cancellationToken);
            }
            catch (Exception ex)
            {
                logger.Debug($"Video {video.DisplayInfo()} has no thumbnail, or we errored - Error: {ex}");
                return new byte[0];
            }
        }
    ...
    {
            var context = youTubeCleanupToolDbContextFactory.Create();
            var playlistItem = await youTubeApi.AddVideoToPlaylist(playlistId, videoId);
    ...
    public async Task RemoveVideoFromPlaylist(string playlistId, string videoId)
    {
            ...
            await youTubeApi.RemoveVideoFromPlaylist(playlistItem.Id);
            context.RemovePlaylistItem(playlistItem);
            await context.SaveChangesAsync();
        }
